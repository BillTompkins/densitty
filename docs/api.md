...
### *class* densitty.plot.Plot(data: Sequence[Sequence[FloatLike]], color_map, render_halfheight, font_mapping, min_data, max_data, x_axis, y_axis, flip_y)
...
#### show(printer=<built-in function print>)
Simple helper function to output/print a plot

#### upscale(max_size: tuple[int, int] = (0, 0), max_expansion: tuple[int, int] = (3, 3), keep_aspect_ratio: bool = False)
- max_size
...
### *class* densitty.axis.Axis(value_range, labels = None, label_fmt = '{}', border_line = False, values_are_edges = False, fractional_tick_pos = False)
- value_range: Tuple/List/util.ValueRange with the Axis limits
- labels: dict of coordinate value -> string to print as axis label. If None, labels will be autogenerated
- label_fmt: For autogenerated labels, specifies the number formatting, e.g. '{:0.2}'
- border_line: Include a line along the axis in the output
- values_are_edges: True for histogram-like data, where labeled values are typically the edges of the histogram bins
- fractional_tick_pos: Use various characters to indicate where an axis tick is, rather than just "-" or "|".
...
### densitty.binning.histogram2d(points, bins = 10, ranges = None, align=True, drop_outside=True, \*\*axis_args)
...
### *class* densitty.detect.ColorSupport(\*values)
...
### *class* densitty.detect.GlyphSupport(\*values)
...
### densitty.detect.color_support(interactive=True, debug=False)
...
Returns [ColorSupport](#densitty.detect.ColorSupport)

### densitty.detect.combining_support(debug=False)
...
### densitty.detect.histplot2d(points, bins = 10, ranges = None, align=True, drop_outside=True, colors=FADE_IN, border_line=True, fractional_tick_pos=False, scale= False, \*\*plotargs)
